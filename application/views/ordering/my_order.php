                        ...
                        <li class="list-group-item d-flex justify-content-between lh-condensed col-md-12 total">
                                <table width="100%">
                                    <tr>
                                        ...
                                        <td width="70%" class='text-right'><h4 class="text-muted gtotal">Rp.  0.00</h4></td>
                                    </tr>
                                </table>
                        </li>
                    </ul>
            ...
            $.ajax({
                ...
                success:resp=>{
                    if(typeof resp != undefined){
                        ...
                        $('#order_details #content-field .gtotal').html('Rp.  '+(parseFloat(resp.details.total_amount).toLocaleString('en-US',{style:'decimal', minimumFractionDigits:2, maximumFractionDigits:2 })))
                        ...
                        resp.list[k].total_amount = resp.list[k].total_amount.replace(/,/g,'')
                                ...
                                li.find('.price').html('Rp. '+parseFloat(resp.list[k].price).toLocaleString('en-US'))
                                li.find('.qty').html('x'+resp.list[k].qty)
                                li.find('.t-Amount').html('Rp.  '+ parseFloat(resp.list[k].total_amount).toLocaleString('en-US',{style:'decimal', minimumFractionDigits:2, maximumFractionDigits:2 }))
                                // console.log(li)
                ...
                if(Object.keys(resp).length >0 ){
                    Object.keys(resp).map((k)=>{
                        var li = $('#order_list_clone .order-list').clone()
                            ...
                            li.find('.total-amount').html('Rp.  '+parseFloat(resp[k].total_amount).toLocaleString('en-US',{ style :'decimal','maximumFractionDigits':2,minimumFractionDigits:2 }))
